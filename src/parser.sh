...
parser_definition() {
	setup _rf_unknown_args help:usage plus:false no:false error:_rf_error -- \
		"Usage: run_for.sh [options]... [arguments]..." ''
	msg -- 'Options:'
	...
	param _rf_command -c --command var:COMMAND init:="" -- "command to be executed (required)"
...
_rf_error() {
	case $2 in
	_rf_validate_seconds:1) printf "Not a valid number: %s %s\n" "$3" "$OPTARG" ;;
	_rf_validate_seconds:2) printf "Script must run for at least 1 second\n" ;;
...
_rf_is_string_empty() {
	awk \
		-v str="$1" \
		-v regex='^\\s*\\S.*$' \
		'BEGIN{ exit match(str, regex) == 0 }' && return 1 # True
	return 0                                            # False
}
...
Options:
  ...
  -c,     --command COMMAND   command to be executed (required)
